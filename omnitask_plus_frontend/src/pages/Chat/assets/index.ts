...
export const getImage = async (user: StreamChatGenerics['user']) => {
...
export const getCleanImage = async (member: ChannelMemberResponse<StreamChatGenerics>) => {
  let cleanImage = member.user?.image || '';
  // const cleanIndex = staticImages.indexOf(cleanImage);
  // if (cleanIndex === -1) {
    // console.log(member.user)
    cleanImage = await getImage(member.user);
  // }
